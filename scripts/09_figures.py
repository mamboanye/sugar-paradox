...
def _panel_label(ax, label, x=-0.08, y=1.05):
    """Bold panel label (A, B) at top-left."""
    ax.text(x, y, label, transform=ax.transAxes,
            fontsize=12, fontweight="bold", va="top", ha="left", color=TEXT)


def _annot_box(ax, text, x, y, **kwargs):
    """Annotation with warm cream background."""
    defaults = dict(
        transform=ax.transAxes, va="top", fontsize=9, color=TEXT,
        bbox=dict(boxstyle="round,pad=0.3", facecolor=ANNOT_BG, edgecolor="none", alpha=0.9),
    )
...
def figure_1_cross_sectional(panel: pd.DataFrame) -> None:
    ...
    _clean_ax(ax)
    _panel_label(ax, "A")
    ax.scatter(x, y, s=35, alpha=0.75, edgecolors=TEXT, linewidths=0.4, c=AMBER)
    m, b = np.polyfit(x, y, 1)
    x_fit = np.linspace(x.min(), x.max(), 100)
    ax.plot(x_fit, m * x_fit + b, color=AMBER, linewidth=1.2)
    r_raw, p_raw = stats.pearsonr(x, y)
    ax.set_xlabel("Sugar supply (kcal/capita/day)")
    ax.set_ylabel("Obesity prevalence (%)")
    _annot_box(ax, f"r = {r_raw:.3f}\np = {p_raw:.4f}", 0.05, 0.95)

    # Panel B: Partial scatter (residualized on GDP + urbanization + oil)
    ax = axes[1]
    _clean_ax(ax)
    _panel_label(ax, "B")
    ax.scatter(x_resid, y_resid, s=35, alpha=0.75, edgecolors=TEXT, linewidths=0.4, c=AMBER)
    m2, b2 = np.polyfit(x_resid, y_resid, 1)
    xr_fit = np.linspace(x_resid.min(), x_resid.max(), 100)
    ax.plot(xr_fit, m2 * xr_fit + b2, color=AMBER, linewidth=1.2)
    ax.set_xlabel("Sugar supply (residualized)")
    ax.set_ylabel("Obesity prevalence (residualized)")
    _annot_box(ax, f"partial r = {r_partial:.3f}\np = {p_partial:.4f}", 0.05, 0.95)
...
def figure_2_fe_cascade(panel: pd.DataFrame) -> None:
    ...
    # First bar (pooled OLS, significant) in AMBER, rest in SLATE
    colors = [AMBER] + [SLATE] * (len(t_values) - 1)

    fig, ax = plt.subplots(figsize=(8, 5))
    _clean_ax(ax)
    bars = ax.bar(range(len(t_values)), t_values, color=colors,
                  edgecolor=TEXT, linewidth=0.5)
    ax.axhline(y=2, color=GRAY, linestyle="--", linewidth=0.6)
    ax.axhline(y=-2, color=GRAY, linestyle="--", linewidth=0.6)
    ax.axhline(y=0, color=TEXT, linewidth=0.5)
    ax.set_xticks(range(len(labels)))
    ax.set_xticklabels(labels, fontsize=9)
    ax.set_ylabel("t-statistic (clustered SE)")

    for i, t in enumerate(t_values):
        ax.text(i, t + 0.1 * np.sign(t), f"{t:.2f}", ha="center",
                va="bottom" if t > 0 else "top", fontsize=9, color=TEXT)
...
def figure_3_global_trend_r2() -> None:
    ...
    fig, axes = plt.subplots(1, 2, figsize=(10, 4.5), gridspec_kw={"width_ratios": [1, 1.2]})

    # Panel A: full histogram
    ax = axes[0]
    _clean_ax(ax)
    _panel_label(ax, "A")
    ax.hist(r2_global, bins=50, color=GRAY, alpha=0.7, edgecolor=TEXT, linewidth=0.3)
    ax.axvline(x=0.90, color=GRAY, linestyle="--", linewidth=0.6)
    ax.set_xlabel("Linear trend R$^2$")
    ax.set_ylabel("Number of countries")
    pct = np.mean(np.array(r2_global) >= 0.90) * 100
    _annot_box(ax, f"{pct:.0f}% $\\geq$ 0.90", 0.35, 0.90)

    # Panel B: zoomed R2 > 0.90, SSA highlighted
    ax = axes[1]
    _clean_ax(ax)
    _panel_label(ax, "B")
    ...
    ax.hist(r2_high, bins=bins_zoom, color=GRAY, alpha=0.5, edgecolor=TEXT,
            linewidth=0.3, label="All countries")
    ax.hist(r2_ssa_high, bins=bins_zoom, color=AMBER, alpha=0.8, edgecolor=TEXT,
...
def figure_4_soybean_oil_case() -> None:
    ...
    tests_main = [
        ("Cross-sectional r", "pass", results["cross_sectional"]["r"]),
        ("Country FE t", "pass", results["country_fe"]["t_cl"]),
        ("TWFE t", "fail", results["twfe"]["t_cl"]),
        ("Lead-lag ratio", "fail", results["lead_lag"]["ratio"]),
        ("Detrended t", "fail", results["detrended"]["t_cl"]),
        ("First differences r", "fail", results["first_differences"]["r"]),
        ("Granger r", "fail", results["granger"]["r"]),
    ]

    fig, axes = plt.subplots(1, 2, figsize=(11, 4.5), gridspec_kw={"width_ratios": [3, 1.2]})

    # Panel A: main diagnostics
    ax = axes[0]
    _clean_ax(ax)
    _panel_label(ax, "A", x=-0.12)
    labels_a = [t[0] for t in tests_main]
    values_a = [t[2] for t in tests_main]
    colors_a = [AMBER if verdict == "pass" else SLATE for _, verdict, _ in tests_main]

    bars = ax.barh(range(len(tests_main)), values_a, color=colors_a,
                   edgecolor=TEXT, linewidth=0.5, height=0.6)
    ax.axvline(x=0, color=TEXT, linewidth=0.5)
    ax.axvline(x=2, color=GRAY, linestyle="--", linewidth=0.6)
    ax.axvline(x=-2, color=GRAY, linestyle="--", linewidth=0.6)
    ...
    for i, (label, verdict, val) in enumerate(tests_main):
        ...
        ax.text(x_pos, i, f"{val:.2f}", va="center", ha=ha, fontsize=9, color=TEXT)
    ...
    ax.legend(handles=[Patch(facecolor=AMBER, edgecolor=TEXT, label="Passes (standard tests)"),
                       Patch(facecolor=SLATE, edgecolor=TEXT, label="Fails (trend diagnostics)")],
              loc="lower right", fontsize=8, frameon=False)

    # Panel B: Oster delta (separate scale)
    ax2 = axes[1]
    _clean_ax(ax2)
    _panel_label(ax2, "B", x=-0.15)
    oster_val = results["oster_bounds"]["delta"]
    ax2.barh([0], [oster_val], color=SLATE, edgecolor=TEXT, linewidth=0.5, height=0.6)
    ax2.barh([1], [3.0], color=LIGHT_GRAY, edgecolor=TEXT, linewidth=0.5, height=0.6)
    ax2.text(oster_val + 0.3, 0, f"{oster_val:.1f}", va="center", fontsize=10,
             fontweight="bold", color=TEXT)
    ax2.text(3.0 + 0.3, 1, "3.0", va="center", fontsize=9, color=GRAY)
...
def figure_5_gdp_control() -> None:
    ...
    _clean_ax(ax)
    _panel_label(ax, "A")
    ax.scatter(cm["log_gdp"], cm["obesity_prevalence_pct"], s=35, alpha=0.75,
               c=SAGE, edgecolors=TEXT, linewidths=0.4)
...
def figure_6_variance_decomposition(panel: pd.DataFrame) -> None:
    ...
    colors = [AMBER, SLATE, GRAY]

    for i, (cat, val, bot, col) in enumerate(zip(categories, values, bottoms, colors)):
        ax.bar(i, val, bottom=bot, color=col, edgecolor=TEXT, linewidth=0.5, width=0.6)
